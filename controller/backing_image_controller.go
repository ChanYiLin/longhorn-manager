...
func (bic *BackingImageController) handleV2BackingImage(bi *longhorn.BackingImage) (err error) {
	...
	if err := bic.deleteInvalidV2Copy(bi); err != nil {
		return errors.Wrapf(err, "failed to delete invalied v2 backing image")
	}
...
func (bic *BackingImageController) prepareFirstV2Copy(bi *longhorn.BackingImage) (err error) {
	...
	// If the first v2 copy is in progress, we wait for the next reconcilation
...
func (bic *BackingImageController) deleteInvalidV2Copy(bi *longhorn.BackingImage) (err error) {
	...
	logrus.Infof("[DEBUG]: DiskFileSpecMap", bi.Spec.DiskFileSpecMap)
	logrus.Infof("[DEBUG]: DiskFileStatusMap", bi.Status.DiskFileStatusMap)

	for v2DiskUUID, copyStatus := range bi.Status.DiskFileStatusMap {
		logrus.Infof("[DEBUG]:check %v %v", v2DiskUUID, copyStatus.State)
		logrus.Infof("[DEBUG] bi.Spec.DiskFileSpecMap[v2DiskUUID]: %v", bi.Spec.DiskFileSpecMap[v2DiskUUID])
	...
	logrus.Infof("[DEBUG]: missingSpecV2Copies %v", missingSpecV2Copies)
	logrus.Infof("[DEBUG]: v2CopiesUnknown %v", v2CopiesUnknown)
	logrus.Infof("[DEBUG]: v2CopiesFailed %v", v2CopiesFailed)
	logrus.Infof("[DEBUG]: hasReadyV2BackingImage %v", hasReadyV2BackingImage)
	...
	for _, v2DiskUUID := range v2CopiesUnknown {
		...
		// Get will return diffrent error if lvol is found on the disk but the record is not found.
...
func (bic *BackingImageController) syncV2StatusWithInstanceManager(bi *longhorn.BackingImage) error {
	...
	for _, im := range ims {
		...
		for biNameDiskUUID, statusInfo := range im.Status.V2BackingImages {
	...
	for diskUUID, copyStatus := range bi.Status.DiskFileStatusMap {
		...
		// Update the status to unknow if the instance manager is unknown or the status is not found from the instance manager
